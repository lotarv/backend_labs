...
public class BatchOmsOrderCreatedConsumer(
    ...
    ILogger<BaseBatchKafkaConsumer<OrderCreatedMessage>> logger)
    : BaseBatchKafkaConsumer<OrderCreatedMessage>(kafkaSettings, kafkaSettings.Value.OmsOrderCreatedTopic, logger)
{
    protected override async Task ProcessMessages(Message<OrderCreatedMessage>[] messages, CancellationToken token)
    {
        ...
        {
            Orders = messages.SelectMany(message => message.Body.OrderItems.Select(ol =>
